...
import { PageBrandArt, type BrandScene } from "./page-brand-art";

const MotionContext = createContext({ paused: false, reduced: true, toggle: () => {} });
const SceneContext = createContext<BrandScene>("home");

export function BrandSceneProvider({ scene, children }: { scene: BrandScene; children: ReactNode }) {
...
type BackdropProps = { variant?: "orbit" | "flow" | "signature"; tone?: "light" | "dark"; className?: string; paused?: boolean; scene?: BrandScene };

/** Each page keeps its own visual identity within one shared motion system. */
export function BrandBackdrop({ variant = "signature", tone = "light", className = "", paused = false, scene: sceneOverride }: BackdropProps) {
  const motion = useContext(MotionContext);
  const pageScene = useContext(SceneContext);
  const scene = sceneOverride ?? pageScene;
  ...
  return <div ref={root} className={`br-backdrop br-${variant} br-${tone} ${className}`} data-scene={scene} data-paused={paused || motion.paused || motion.reduced} data-active="false" aria-hidden="true">
    {scene === "home" ? <>
    <div className="br-mark"><QuantLogo compact inverse={tone === "dark"} /></div>
    <div className="br-orbit-rings"><i /><i /><span /></div>
    ...
    </> : <PageBrandArt scene={scene} tone={tone} />}
  </div>;
}
