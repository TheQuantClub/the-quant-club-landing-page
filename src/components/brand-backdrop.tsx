...
export function BrandBackdrop({ variant = "signature", tone = "light", className = "", paused = false }: BackdropProps) {
  ...
  return <div ref={root} className={`br-backdrop br-${variant} br-${tone} ${className}`} data-paused={paused || motion.paused || motion.reduced} data-active="false" aria-hidden="true">
    ...
    <svg className="br-streams" viewBox="0 0 1440 140" preserveAspectRatio="none">
      ...
      {[0, 1, 2].map(index => <g key={index} transform={`translate(0 ${index * 17})`}><path d="M-80 36C350 2 580 114 970 67S1320 18 1520 43" fill="none" stroke={`url(#${id}-brand-line)`} strokeWidth={index === 1 ? "1.4" : ".8"} vectorEffect="non-scaling-stroke" /><path className="br-trace" d="M-80 36C350 2 580 114 970 67S1320 18 1520 43" fill="none" stroke={tone === "dark" ? "#5dbbff" : "#2878f0"} strokeWidth="2" vectorEffect="non-scaling-stroke" pathLength="1000" strokeDasharray="42 958" style={{ animationDelay: `${index * -4}s` }} /></g>)}
    </svg>
  </div>;
}
