...
function Strategies() {
  ...
  const selectedIndex = publicStrategies.indexOf(selected);
  ...
  return <>
    <section className="mk-collection-intro tqc-container"><div className="mk-breadcrumb"><Link href="/">Home</Link><ChevronRight size={13} aria-hidden="true" /><span>Strategies</span></div><div><div><p className="tqc-eyebrow">THE STRATEGY COLLECTION</p><h1>Distinct universes.<br /><em>One quantitative discipline.</em></h1></div><p>Explore equity and mutual fund strategies for your practice. Find your investment universe, then go deeper within the platform.</p></div></section>
    <section className="mk-collection-section tqc-container" aria-label="Explore the strategy collection">
      ...
      <div className="mk-collection-explorer">
        <div className="mk-collection-list" role="tablist" aria-label="Choose a strategy" aria-orientation="vertical">{filtered.map((strategy, index) => <button type="button" key={strategy.slug} id={`mk-collection-tab-${strategy.slug}`} role="tab" aria-selected={selected.slug === strategy.slug} aria-controls="mk-collection-panel" tabIndex={selected.slug === strategy.slug ? 0 : -1} onClick={() => setSelectedSlug(strategy.slug)} onKeyDown={event => moveStrategy(event, index)} ref={element => { tabRefs.current[index] = element; }}><span className="mk-collection-number">{String(publicStrategies.indexOf(strategy) + 1).padStart(2, "0")}</span><span><small>{strategy.type}</small>{strategy.name}</span><ArrowUpRight size={19} aria-hidden="true" /></button>)}</div>
        <div className="mk-collection-stage" id="mk-collection-panel" role="tabpanel" aria-labelledby={`mk-collection-tab-${selected.slug}`} tabIndex={0}>
          <div className="mk-stage-top"><span>{selected.type === "Equity" ? "EQUITY STRATEGY" : "MUTUAL FUND STRATEGY"}</span><span>THE QUANT CLUB / {String(selectedIndex + 1).padStart(2, "0")}</span></div>
          <div className="mk-stage-main" key={selected.slug}><div className="mk-stage-title"><span>{selected.universe}</span><h2>{selected.name}</h2></div><div className="mk-stage-sculpture" aria-hidden="true"><StrategyCharacterArt variant={selected.shape} tone="dark" paused={paused} /></div></div>
...
function StrategyDetail({ slug }: { slug: string }) {
  ...
  const [paused, setPaused] = useState(false);
  return <>
    <section className={`mk-strategy-detail-hero mk-strategy-detail-${strategy.shape}`}><div className="tqc-container"><div className="mk-breadcrumb"><Link href="/strategies"><ArrowLeft size={14} aria-hidden="true" />The strategy collection</Link><span>{String(strategyIndex + 1).padStart(2, "0")} / {String(publicStrategies.length).padStart(2, "0")}</span></div><div className="mk-strategy-detail-grid"><div><p className="tqc-eyebrow">{strategy.type === "Equity" ? "EQUITY STRATEGY" : "MUTUAL FUND STRATEGY"}</p><h1>{strategy.name}</h1><p>{strategy.description}</p><WalkthroughLink interest={strategy.name} className="mk-light-button">Discuss this strategy</WalkthroughLink></div><div className="mk-detail-sculpture"><div aria-hidden="true"><StrategyCharacterArt variant={strategy.shape} tone="dark" paused={paused} /></div><div className="mk-sculpture-caption"><span>{strategy.universe}</span><button type="button" aria-pressed={paused} onClick={() => setPaused(!paused)}>{paused ? <Play size={13} aria-hidden="true" /> : <Pause size={13} aria-hidden="true" />}{paused ? "Play" : "Pause"} animation</button></div></div></div><div className="mk-strategy-detail-meta"><span><i />{strategy.universe}</span><span>RESEARCH / ANALYSIS / IMPLEMENTATION</span></div></div></section>
...
const workflowStages = [
  { id: "research", name: "Model portfolios", icon: Layers3, title: "A defined starting point for your investment process.", copy: "Explore model portfolios within the platform, supported by quantitative research. Assess how each fits the mandate your firm has defined, with the supporting context close at hand.", tasks: ["Your investment mandate", "Model portfolio access", "Supporting research"], output: "The strategy and its context, together" },
...
const audiences = [
  { id: "advisers", name: "Investment advisers", short: "RIAs & advisory teams", icon: Users, title: "Carry your process through to the client.", description: "Bring systematic research into your practice, prepare implementation across client portfolios, and keep reviews and follow-through organised. Present the work under your firm’s brand.", uses: ["Evaluate research against the mandate you have defined.", "Prepare client-wise implementation and rebalance instructions.", "Keep review work and branded client communication connected."], note: "Your team retains suitability decisions, allocation and execution." },
  { id: "distributors", name: "Mutual fund distributors", short: "MFDs & fund-focused teams", icon: Layers3, title: "Make the work around fund decisions more consistent.", description: "Connect mutual fund research with implementation preparation, SIP follow-through and portfolio reviews. Bring the process into clear communication under your own brand.", uses: ["Explore mutual fund research relevant to your distribution practice.", "Organise implementation preparation and ongoing portfolio review.", "Prepare consistent factsheets and review material for client conversations."], note: "Research and tools are considered within your firm’s distribution role." },
  { id: "wealth", name: "Wealth management teams", short: "INVESTMENT, OPERATIONS & RELATIONSHIP TEAMS", icon: Users, title: "Keep the whole team working from the same context.", description: "Connect research, client-wise implementation work and ongoing reviews across your firm. Give relationship teams clear material to carry into the next client conversation.", uses: ["Give investment and operations teams a shared research reference.", "Connect implementation preparation, confirmations and review actions.", "Bring consistent factsheets and client material under your firm’s brand."], note: "Your team retains its client relationships, investment decisions and execution." },
  { id: "institutions", name: "Investment institutions", short: "PMS, AIFs & family offices", icon: SlidersHorizontal, title: "Add research where your team needs depth.", description: "Bring quantitative research into your existing investment process. Use your own systems for implementation and reporting, or discuss the workflow support that fits your team.", uses: ["Evaluate research alongside your mandate and internal review process.", "Carry the research into your existing implementation workflow.", "Discuss ongoing review, branded reporting and team requirements."], note: "Access and the scope of support are agreed in the context of your firm." },
] as const;

function Institutions() {
  return <>
    <PageIntro label="WHO WE SERVE" title={<>Your practice.<br /><em>A process that fits.</em></>} description="Different firms need different support. Start with your research needs, then connect the implementation, maintenance and communication work around them." />
    ...
    <section className="mk-tint-section"><div className="tqc-container mk-two-column"><div><p className="tqc-eyebrow">ONE CONNECTED WORKFLOW</p><h2>Research is the start.<br /><em>Your practice brings it to life.</em></h2></div><div><p className="mk-body-lead">Model portfolios, the analysis engine, implementation, maintenance, your brand and client communication.</p><p>We begin by understanding how your firm works today. The next conversation is about where a clearer process and the right tools can help.</p><Link className="tqc-text-link" href="/platform">Explore the platform<ArrowRight size={17} aria-hidden="true" /></Link></div></div></section><StrategyCTA />
  </>;
}
...
function About() {
  return <>
    ...
    <section className="mk-section"><div className="tqc-container mk-about-story"><div className="mk-about-emblem" aria-hidden="true"><i /><i /><i /><QuantLogo compact /><span>THE QUANT CLUB</span></div><div><p className="tqc-eyebrow">WHY WE EXIST</p><h2>An investment process<br />has more than one job.</h2><p className="mk-body-lead">It needs to guide a decision, support the work of implementation, and hold together through ongoing reviews and client conversations.</p><p>That is the connection we focus on. Research that can be examined. Workflows that bring the next action into view. Communication that keeps your firm’s identity at the centre.</p><p>We work around the professional’s role: your firm understands the client, makes the investment decisions and carries out execution.</p></div></div></section>
