...
function StrategyDetail({ slug }: { slug: string }) {
  ...
  const [paused, setPaused] = useState(false);
  return <>
    <section className={`mk-strategy-detail-hero mk-brand-surface mk-strategy-detail-${strategy.shape}`}><BrandBackdrop variant="flow" tone="dark" className="mk-surface-backdrop mk-dark-backdrop" /><div className="tqc-container"><div className="mk-breadcrumb"><Link href="/strategies"><ArrowLeft size={14} aria-hidden="true" />The strategy collection</Link><span>{strategy.type}</span></div><div className="mk-strategy-detail-grid"><div><p className="tqc-eyebrow">{strategy.type === "Equity" ? "EQUITY STRATEGY" : "MUTUAL FUND STRATEGY"}</p><h1>{strategy.name}</h1><p>{strategy.description}</p><WalkthroughLink interest={strategy.name} className="mk-light-button">Discuss this strategy</WalkthroughLink></div><div className="mk-detail-sculpture"><div aria-hidden="true"><StrategyCharacterArt variant={strategy.shape} tone="dark" paused={paused} /></div><div className="mk-sculpture-caption"><span>{strategy.universe}</span><button type="button" aria-pressed={paused} onClick={() => setPaused(!paused)}>{paused ? <Play size={13} aria-hidden="true" /> : <Pause size={13} aria-hidden="true" />}{paused ? "Play" : "Pause"} animation</button></div></div></div><div className="mk-strategy-detail-meta"><span><i />{strategy.universe}</span><span>RESEARCH / ANALYSIS / IMPLEMENTATION</span></div></div></section>
...
const audiences = [
  { id: "advisers", name: "Investment advisers", short: "RIAs & advisory teams", icon: Users, benefit: "Research through to client review", title: "Carry your process through to the client.", description: "Connect model portfolios and analysis with implementation, ongoing reviews and communication in your firm’s name.", uses: ["Evaluate strategies against your mandate.", "Prepare client-specific order files and review actions.", "Bring research and reporting into your brand."], note: "Your team retains suitability decisions, allocation and execution." },
  { id: "distributors", name: "Mutual fund distributors", short: "MFDs & fund-focused teams", icon: Layers3, benefit: "A consistent fund workflow", title: "Make the work around fund decisions more consistent.", description: "Support your fund offering with research, implementation preparation, SIP follow-through and clear client material.", uses: ["Explore research for your mutual fund offering.", "Organise implementation and portfolio review.", "Prepare factsheets in your firm’s identity."], note: "Research and tools are considered within your firm’s distribution role." },
  { id: "wealth", name: "Wealth teams", short: "Investment, operations & relationship teams", icon: Users, benefit: "Connect the people and the process", title: "Keep the whole team working from the same context.", description: "Bring investment, operations and relationship teams around a shared process, from analysis to the client conversation.", uses: ["Work from the same research and analysis.", "Connect implementation records and open actions.", "Keep client material consistent across the firm."], note: "Your team retains its client relationships, investment decisions and execution." },
  { id: "institutions", name: "Investment institutions", short: "PMS, AIFs & family offices", icon: SlidersHorizontal, benefit: "Research for your existing systems", title: "Add research where your team needs depth.", description: "Bring quantitative research into your existing investment process. Use your own systems or discuss the workflow support you need.", uses: ["Assess strategies alongside your internal research.", "Use the research within your implementation process.", "Discuss analysis, reporting and team requirements."], note: "Access and the scope of support are agreed in the context of your firm." },
] as const;
...
function About() {
  return <>
    <PageIntro variant="signature" label="ABOUT THE QUANT CLUB" title={<>Discipline in the research.<br /><em>Care in the work that follows.</em></>} description="The Quant Club brings quantitative research and practical workflows together for India’s investment professionals." />
