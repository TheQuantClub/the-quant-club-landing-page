...
import { ArrowLeft, ArrowRight, ArrowUpRight, BookOpen, Check, ChevronRight, ClipboardCheck, Download, FileText, Fingerprint, Layers3, LockKeyhole, Mail, Menu, Plus, RefreshCw, Search, ShieldCheck, SlidersHorizontal, Users, Workflow, X } from "lucide-react";
...
function PageIntro({ label, title, description, children }: { label: string; title: ReactNode; description: string; children?: ReactNode }) {
  return <section className="mk-page-intro"><div className="tqc-container"><div className="mk-breadcrumb"><Link href="/">Home</Link><ChevronRight size={13} aria-hidden="true" /><span>{label}</span></div><div className={`mk-intro-grid${children ? "" : " mk-intro-simple"}`}><div><p className="tqc-eyebrow">{label}</p><h1>{title}</h1><p className="mk-lead">{description}</p></div>{children}</div></div></section>;
}

function StrategyMotif({ index = 0 }: { index?: number }) {
  return <div className={`mk-strategy-motif mk-strategy-motif-${index % 3}`} aria-hidden="true"><i /><i /><i /><i /><span /></div>;
}
...
function Strategies() {
  ...
  return <>
    <PageIntro label="STRATEGIES" title={<>A defined universe.<br /><em>A considered approach.</em></>} description="Explore the investment areas our research covers. Start with the mandate your firm needs, then discuss the research and workflow behind it." />
    <section className="mk-section mk-strategy-section"><div className="tqc-container">
      <div className="mk-filter-row"><div className="mk-filters" role="group" aria-label="Filter strategies">{["All strategies", "Equity", "Mutual funds"].map(value => <button type="button" key={value} aria-pressed={filter === value} onClick={() => setFilter(value)}>{value}</button>)}</div><span className="mk-access-inline"><LockKeyhole size={15} aria-hidden="true" />Member research stays private</span></div>
      <div className="mk-strategy-grid">{filtered.map(strategy => <Link className="mk-strategy-card" href={`/strategies/${strategy.slug}`} key={strategy.slug}><div className="mk-strategy-card-top"><span className="mk-tag">{strategy.type}</span><ArrowUpRight size={22} aria-hidden="true" /></div><StrategyMotif index={publicStrategies.indexOf(strategy)} /><span className="mk-card-kicker">{strategy.universe}</span><h2>{strategy.name}</h2><p>{strategyStories[strategy.slug]?.use || strategy.description}</p><span className="tqc-text-link">Explore the scope<ArrowRight size={17} aria-hidden="true" /></span></Link>)}</div>
      <div className="mk-context-note"><LockKeyhole size={21} aria-hidden="true" /><div><h3>The introduction is public. The research is private.</h3><p>Holdings, model details and portfolio information are not displayed here. Access is discussed in the context of your firm’s role and requirements.</p></div></div>
    </div></section><StrategyCTA />
  </>;
}
...
function StrategyDetail({ slug }: { slug: string }) {
  ...
  return <>
    <PageIntro label="STRATEGY INTRODUCTION" title={<>{strategy.name}<br /><em>{strategy.universe}.</em></>} description={strategy.description}><div className="mk-intro-motif"><StrategyMotif index={publicStrategies.indexOf(strategy)} /><span>THE QUANT CLUB / {strategy.type.toUpperCase()}</span></div></PageIntro>
    <section className="mk-section"><div className="tqc-container mk-detail-layout"><div><Link className="tqc-text-link" href="/strategies"><ArrowLeft size={17} aria-hidden="true" />Back to strategies</Link><p className="tqc-eyebrow">WHERE THE RESEARCH FITS</p><h2>{strategy.title}</h2><p className="mk-body-lead">{strategyStories[slug]?.use || strategy.lens}</p><p>Begin with your firm’s investment mandate. A walkthrough is the place to discuss the research, how you would evaluate it, and the deployment and ongoing review support your team needs.</p><div className="mk-detail-topics">{[[BookOpen, "Understand the scope", "Discuss the investment universe and the role of the research in your practice."], [Workflow, "Connect it to your workflow", "Explore deployment preparation, review processes and client communication."], [Users, "Keep your firm in control", "Your team retains client relationships, suitability decisions and execution."]].map(([Icon, title, copy]) => { const TopicIcon = Icon as typeof BookOpen; return <div key={String(title)}><TopicIcon size={23} aria-hidden="true" /><div><h3>{String(title)}</h3><p>{String(copy)}</p></div></div>; })}</div></div><aside className="mk-detail-aside"><LockKeyhole size={27} aria-hidden="true" /><p className="tqc-eyebrow">A CONVERSATION WITH CONTEXT</p><h3>Explore what your<br />firm needs to know.</h3><p>Detailed research is kept within authorised member access. The public website does not display holdings, model information or performance figures.</p><WalkthroughLink interest={strategy.name}>Discuss this strategy</WalkthroughLink><small>A systematic process does not guarantee an investment outcome.</small></aside></div></section><StrategyCTA />
  </>;
}

const workflowStages = [
  { name: "Research", icon: BookOpen, title: "Start with a defined process.", copy: "Bring a consistent research framework to the investment areas your firm follows. Keep the questions, context and documented process connected.", tasks: ["Research context", "Defined approach", "Dated research record"], output: "A shared research foundation" },
  { name: "Evaluate", icon: SlidersHorizontal, title: "Make room for due diligence.", copy: "Examine the research in the context of your mandate. Discuss its assumptions, limitations and evidence before deciding how it belongs in your practice.", tasks: ["Mandate review", "Research questions", "Team assessment"], output: "A decision your team can explain" },
  { name: "Deploy", icon: Workflow, title: "Turn a decision into an organised workflow.", copy: "Prepare client-wise deployment and rebalance instructions. Your firm executes through its own broker or mutual fund platform, then records the outcome.", tasks: ["Deployment preparation", "Firm-led execution", "Confirmation and follow-up"], output: "Clear instructions and follow-through" },
  { name: "Maintain", icon: RefreshCw, title: "Keep track of the work after deployment.", copy: "Organise ongoing review, drift checks, recorded changes and outstanding actions. Give your team a common view of what needs attention.", tasks: ["Review schedule", "Changes and drift", "Outstanding actions"], output: "An organised ongoing review" },
  { name: "Brand", icon: Fingerprint, title: "Keep your firm at the centre.", copy: "Bring your identity to factsheets and review documents. Use your logo and firm details with the appropriate research context and disclosures.", tasks: ["Your firm’s identity", "Consistent document format", "Required disclosures"], output: "Client documents under your brand" },
  { name: "Communicate", icon: Users, title: "Carry the process into the conversation.", copy: "Bring research context, review notes and next steps into clear client communication. Help your team explain the work behind a decision.", tasks: ["Review preparation", "Clear research context", "Client communication"], output: "A more consistent client conversation" },
] as const;
...
function WorkflowPreview() {
  ...
  useEffect(() => {
    let frame = 0;
    function readHash() {
      const index = workflowStages.findIndex(item => `#${item.name.toLowerCase()}` === window.location.hash);
      if (index < 0) return;
      setActive(index);
  ...
  function selectStage(index: number) {
    setActive(index);
    window.history.replaceState(null, "", `#${workflowStages[index].name.toLowerCase()}`);
  }
  ...
  return <div className="mk-workspace" id="workflow">{workflowStages.map(item => <span className="mk-workspace-anchor" id={item.name.toLowerCase()} key={item.name} aria-hidden="true" />)}<div className="mk-workspace-top"><span><i />THE QUANT CLUB / THE WORKFLOW</span><span>Illustrative preview</span></div><div className="mk-workspace-tabs" role="tablist" aria-label="Explore the workflow">{workflowStages.map((item, index) => <button type="button" key={item.name} role="tab" id={`mk-workflow-tab-${index}`} aria-controls="mk-workflow-panel" aria-selected={active === index} tabIndex={active === index ? 0 : -1} onClick={() => selectStage(index)} onKeyDown={event => moveTab(event, index)} ref={element => { tabRefs.current[index] = element; }}><item.icon size={17} aria-hidden="true" />{item.name}</button>)}</div><div className="mk-workspace-body" id="mk-workflow-panel" role="tabpanel" aria-labelledby={`mk-workflow-tab-${active}`} tabIndex={0}><div className="mk-workspace-copy"><span className="mk-workspace-icon"><StageIcon size={30} aria-hidden="true" /></span><p className="tqc-eyebrow">{stage.name.toUpperCase()}</p><h2>{stage.title}</h2><p>{stage.copy}</p></div><div className="mk-workspace-art" aria-label={`${stage.name} workflow illustration`}><div className="mk-task-heading"><span>{stage.name} workspace</span><span className="mk-tag">Illustration</span></div>{stage.tasks.map((task, index) => <div className="mk-task" key={task}><span className="mk-task-check"><Check size={15} aria-hidden="true" /></span><div><span>{task}</span><i style={{ width: `${68 - index * 14}%` }} /></div><ChevronRight size={15} aria-hidden="true" /></div>)}<div className="mk-task-output"><ClipboardCheck size={23} aria-hidden="true" /><span>{stage.output}</span></div></div></div><div className="mk-workspace-foot"><LockKeyhole size={14} aria-hidden="true" /><span>A view of the workflow. No member research or portfolio data is displayed.</span></div></div>;
}

function Platform() {
  return <>
    <PageIntro label="THE PLATFORM" title={<>From research<br /><em>to the work that follows.</em></>} description="A connected workflow for evaluating research, preparing deployment, maintaining portfolios and communicating under your firm’s brand." />
    <section className="mk-workspace-section"><div className="tqc-container"><WorkflowPreview /></div></section>
    <section className="mk-section"><div className="tqc-container"><div className="tqc-section-heading"><div><p className="tqc-eyebrow">BUILT AROUND YOUR PRACTICE</p><h2>The process stays connected.<br /><em>Your firm stays in control.</em></h2></div><p>Take the research into your own systems, or discuss the workflow support your team needs.</p></div><div className="mk-feature-columns"><article><Workflow size={29} aria-hidden="true" /><h3>Deployment with follow-through</h3><p>Prepare instructions, organise client-wise changes and keep confirmation work visible. Execution remains with your firm.</p></article><article><RefreshCw size={29} aria-hidden="true" /><h3>Maintenance with context</h3><p>Keep reviews, changes, recorded flows and open actions connected to the same process.</p></article><article><Fingerprint size={29} aria-hidden="true" /><h3>Communication under your brand</h3><p>Bring the work into consistent factsheets and review documents carrying your firm’s identity.</p></article></div></div></section>
    <section className="mk-tint-section"><div className="tqc-container mk-two-column"><div><p className="tqc-eyebrow">YOUR CLIENT RELATIONSHIPS</p><h2>The relationship<br /><em>belongs to your firm.</em></h2></div><div><p className="mk-body-lead">Client decisions, suitability and execution stay with your team.</p><p>The client workflow is designed around anonymous reference tags. End-investor names, PANs and contact details stay with the firm, with local personalisation for client documents.</p><Link href="/institutions" className="tqc-text-link">See how it fits your practice<ArrowRight size={17} aria-hidden="true" /></Link></div></div></section><StrategyCTA />
  </>;
}

const audiences = [
  { id: "advisers", name: "Investment advisers", short: "RIAs & advisory teams", icon: Users, title: "Carry your process through to the client.", description: "Bring systematic research into your practice, prepare deployment across client portfolios, and keep reviews and follow-through organised. Present the work under your firm’s brand.", uses: ["Evaluate research against the mandate you have defined.", "Prepare client-wise deployment and rebalance instructions.", "Keep review work and branded client communication connected."], note: "Your team retains suitability decisions, allocation and execution." },
  { id: "distributors", name: "Mutual fund distributors", short: "MFDs & fund-focused teams", icon: Layers3, title: "Make the work around fund decisions more consistent.", description: "Connect mutual fund research with deployment preparation, SIP follow-through and portfolio reviews. Bring the process into clear communication under your own brand.", uses: ["Explore mutual fund research relevant to your distribution practice.", "Organise deployment preparation and ongoing portfolio review.", "Prepare consistent factsheets and review material for client conversations."], note: "Research and tools are considered within your firm’s distribution role." },
  { id: "wealth", name: "Wealth management teams", short: "INVESTMENT, OPERATIONS & RELATIONSHIP TEAMS", icon: Users, title: "Keep the whole team working from the same context.", description: "Connect research, client-wise deployment work and ongoing reviews across your firm. Give relationship teams clear material to carry into the next client conversation.", uses: ["Give investment and operations teams a shared research reference.", "Connect deployment preparation, confirmations and review actions.", "Bring consistent factsheets and client material under your firm’s brand."], note: "Your team retains its client relationships, investment decisions and execution." },
  { id: "institutions", name: "Investment institutions", short: "PMS, AIFs & family offices", icon: SlidersHorizontal, title: "Add research where your team needs depth.", description: "Bring quantitative research into your existing investment process. Use your own systems for deployment and reporting, or discuss the workflow support that fits your team.", uses: ["Evaluate research alongside your mandate and internal review process.", "Carry the research into your existing deployment workflow.", "Discuss ongoing review, branded reporting and team requirements."], note: "Access and the scope of support are agreed in the context of your firm." },
] as const;

function Institutions() {
  return <>
    <PageIntro label="WHO WE SERVE" title={<>Your practice.<br /><em>A process that fits.</em></>} description="Different firms need different support. Start with your research needs, then connect the deployment, maintenance and communication work around them." />
    <div className="tqc-container mk-audience-nav" role="navigation" aria-label="Choose your type of firm">{audiences.map(audience => <a href={`#${audience.id}`} key={audience.id}><audience.icon size={22} aria-hidden="true" /><span>{audience.name}</span><ArrowRight size={17} aria-hidden="true" /></a>)}</div>
    <section className="mk-section"><div className="tqc-container mk-audience-chapters">{audiences.map(audience => <article id={audience.id} className="mk-audience-chapter" key={audience.id}><div><p className="tqc-eyebrow">{audience.short}</p><h2>{audience.title}</h2><p>{audience.description}</p><WalkthroughLink className="tqc-text-link">Discuss your practice</WalkthroughLink></div><div className="mk-audience-use"><h3>Where the workflow helps</h3><ul>{audience.uses.map(use => <li key={use}><Check size={18} aria-hidden="true" /><span>{use}</span></li>)}</ul><p><ShieldCheck size={19} aria-hidden="true" />{audience.note}</p></div></article>)}</div></section>
    <section className="mk-tint-section"><div className="tqc-container mk-two-column"><div><p className="tqc-eyebrow">ONE CONNECTED WORKFLOW</p><h2>Research is the start.<br /><em>Your practice brings it to life.</em></h2></div><div><p className="mk-body-lead">Research, evaluation, deployment, maintenance, branding and communication.</p><p>We begin by understanding how your firm works today. The next conversation is about where a clearer process and the right tools can help.</p><Link className="tqc-text-link" href="/platform">Explore the platform<ArrowRight size={17} aria-hidden="true" /></Link></div></div></section><StrategyCTA />
  </>;
}
...
function Research() {
  ...
  return <>
    <PageIntro label="IDEAS FOR A MORE CONSIDERED PRACTICE" title={<>The Quant <em>Bytes.</em></>} description="Short perspectives on systematic investing, the work around a portfolio, and the conversations that connect them." />
    <section className="mk-journal-section tqc-container">
...
function ArticleDetail({ slug }: { slug: string }) {
  ...
  return <>
    <section className="mk-article-intro tqc-container"><Link className="tqc-text-link" href="/research"><ArrowLeft size={17} aria-hidden="true" />Back to The Quant Bytes</Link><div className="mk-article-meta"><span>{article.category}</span><ArticleStatus article={article} /></div><h1>{article.title}</h1><p className="mk-lead">{article.dek}</p><div className="mk-byline"><QuantLogo compact /><span>{article.author || "The Quant Bytes"}{article.publishedAt ? <small>{article.publishedAt}</small> : <small>{article.status === "draft" ? "Editorial draft for review" : "Perspectives from The Quant Club"}</small>}</span></div><ArticleArtwork art={article.art} large /></section>
    <section className="mk-section"><div className="tqc-container mk-reading-layout"><aside><span className="tqc-eyebrow">ON THIS PAGE</span>{article.sections.map((section, index) => <a href={`#article-section-${index}`} key={section.title}>{section.title}<ChevronRight size={15} aria-hidden="true" /></a>)}</aside><article className="mk-reading-body">{article.status === "draft" && <p className="mk-reading-note">This is an editorial draft for review, not a published company research report.</p>}{article.sections.map((section, index) => <section id={`article-section-${index}`} key={section.title}><h2>{section.title}</h2>{section.paragraphs.map((paragraph, paragraphIndex) => <p key={paragraphIndex}>{paragraph}</p>)}</section>)}<div className="mk-article-close"><p className="tqc-eyebrow">CONTINUE THE CONVERSATION</p><h3>Bring the question to your practice.</h3><p>Explore how research, deployment and ongoing portfolio work fit together for your firm.</p><WalkthroughLink /></div></article></div></section>
    <section className="mk-related mk-tint-section"><div className="tqc-container"><div className="tqc-section-heading"><div><p className="tqc-eyebrow">THE QUANT BYTES</p><h2>Keep thinking.</h2></div><Link className="tqc-text-link" href="/research">View all articles<ArrowRight size={17} aria-hidden="true" /></Link></div><div className="mk-article-grid">{articles.filter(item => item.slug !== slug).slice(0, 3).map(item => <ArticleCard article={item} key={item.slug} />)}</div></div></section>
  </>;
}

function About() {
  return <>
    <PageIntro label="ABOUT THE QUANT CLUB" title={<>Discipline in the research.<br /><em>Care in the work that follows.</em></>} description="The Quant Club brings quantitative research and practical workflows together for India’s investment professionals." />
    <section className="mk-section"><div className="tqc-container mk-about-story"><div className="mk-about-emblem" aria-hidden="true"><i /><i /><i /><QuantLogo compact /><span>THE QUANT CLUB</span></div><div><p className="tqc-eyebrow">WHY WE EXIST</p><h2>An investment process<br />has more than one job.</h2><p className="mk-body-lead">It needs to guide a decision, support the work of deployment, and hold together through ongoing reviews and client conversations.</p><p>That is the connection we focus on. Research that can be examined. Workflows that bring the next action into view. Communication that keeps your firm’s identity at the centre.</p><p>We work around the professional’s role: your firm understands the client, makes the investment decisions and carries out execution.</p></div></div></section>
    <section className="mk-tint-section"><div className="tqc-container"><div className="tqc-section-heading"><div><p className="tqc-eyebrow">WHAT GUIDES THE WORK</p><h2>A clear process.<br /><em>A considered practice.</em></h2></div></div><div className="mk-feature-columns"><article><SlidersHorizontal size={28} aria-hidden="true" /><h3>Rules with a purpose</h3><p>Define the research process clearly. Keep its assumptions and limitations part of the conversation.</p></article><article><Workflow size={28} aria-hidden="true" /><h3>Follow-through matters</h3><p>Connect research to deployment preparation, ongoing review and the practical work of a team.</p></article><article><Fingerprint size={28} aria-hidden="true" /><h3>Your practice comes first</h3><p>Support the firm’s process, brand and client relationships. Keep responsibility and access clear.</p></article></div></div></section>
    <section className="mk-section"><div className="tqc-container mk-two-column"><div><p className="tqc-eyebrow">THE DATA BEHIND THE WORK</p><h2>Credibility begins<br /><em>with the foundations.</em></h2></div><div className="mk-evidence-list"><div><h3>Established data sources</h3><p>Our research uses data from NSE, AMFI, Morningstar and other paid research tools.</p></div><div><h3>Independent examination</h3><p>Our data and backtests are audited by a chartered accountancy firm.</p></div><div><h3>A live record</h3><p>Live data is tracked through PaRRVA. This does not imply a guarantee of returns or regulatory endorsement.</p></div></div></div></section><StrategyCTA />
  </>;
}
