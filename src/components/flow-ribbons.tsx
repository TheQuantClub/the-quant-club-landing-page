...
// Identical shallow curves keep their spacing throughout the lower hero strip.
const paths = [0, 18, 36].map(offset =>
  `M-160 ${64 + offset}C170 ${44 + offset} 430 ${91 + offset} 760 ${70 + offset}S1270 ${46 + offset} 1600 ${64 + offset}`,
);
...
export function FlowRibbons({ paused = false, tone = "light" }: { paused?: boolean; tone?: "light" | "dark" }) {
  ...
  return <div ref={root} className="q-flow-ribbons" data-paused={paused} data-tone={tone} aria-hidden="true"><svg viewBox="0 0 1440 120" preserveAspectRatio="none"><defs>
