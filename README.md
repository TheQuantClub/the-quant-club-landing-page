...
- `/`: the chosen headline and an abstract globe introducing the quant framework on a navy/blue/cyan gradient; quant-led model portfolios and five supporting capabilities on a moving semicircle, “Why The Quant Club” comparisons, a gradient statement about preparing for uncertainty, four audience workflows, The Quant Bytes, FAQs and walkthrough links.
- `/platform`: Quant-led model portfolios, Analysis engine, Implementation, Maintenance, Your brand and Client communication. Each capability supports stable direct hash links and keyboard navigation.
...
- `src/components/brand-backdrop.tsx`: recurring Q marks and three flowing blue lines in their original positions, with a shared background-motion control that persists across page navigation. The ornamental rings around the Q have been removed.
...
- `src/components/quant-framework.tsx`, `quant-globe.tsx` and `flow-ribbons.tsx`: an abstract market-universe-to-rules illustration, with a transparent wire globe and the flowing background masked behind it. The hero shares pause control, offscreen pausing and reduced-motion support; no live investment data is depicted.
...
- `src/app/brand-refresh.css`, `marketing-refresh.css`, `research-problems.css`, `kinetic-refresh.css`, `audience-section.css`, `brand-continuity.css`, `hero-gradient.css`, `adaptive-header.css`, `quant-framework.css`, `statement-section.css` and `strategy-character-art.css`: current styling, alongside base `globals.css`.
